...
\begin{itemize}[leftmargin=18pt,itemsep=2pt]
  ...
  \item The specific thresholds, intervals, and capacity numbers the operational-health watchdog triggers on. The method (watch a trend rather than a snapshot, alert before the crash rather than after) is in this writeup; the operator-tuned numbers are not, because they describe the exact headroom of specific hosts.
  \item The physical escrow arrangement for the backup encryption key, beyond the fact that an off-fleet copy exists and has been verified current. Where it is kept, and how, is out.
  \item The internals of the orchestration and coordination layer the project now runs on: the specific tools, the governance thresholds, and the change-class table that decides what runs unattended. The design reasoning is in the Overview and Scripts documents; the mechanism is not.
  \item The destination of the planned hardware relocation, beyond the fact that it is a move to a new site behind a new upstream provider. It is described only as a forcing function for recovery, never by where it is going.
\end{itemize}
...
\begin{itemize}[leftmargin=18pt,itemsep=2pt]
  ...
  \item The operational-health and capacity methodology: that it is a distinct audit dimension, that it is checked continuously rather than at a point in time, and the reasoning for why a security audit alone does not cover it.
  \item The recovery-verification methodology: proving the backups actually restore rather than assuming they do, escrowing the recovery key off the fleet and verifying that copy is current, and alarming on backup staleness.
  \item My professional identity: name, field of study, country, the fact that this is a personal project of mine.
\end{itemize}
